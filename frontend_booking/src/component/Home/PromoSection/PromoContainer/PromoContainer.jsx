...
const PromoContainer = ({
  ...
  return (
    <article
      className={`flex flex-col items-center gap-4 border-2 p-4 md:p-8 m-4 border-black rounded-xl text-inter text-dark-gray ${className}`}
      {...props}
    >
      ...
      {displayButton ? (
        <BorderButton
          onClick={onClick}
          className={`flex flex-row items-center justify-around gap-6 font-bold text-black border border-black bg-pure-white shadow-rounded-button-yellow
                my-5 mb-0
                px-2 text-xs py-2
